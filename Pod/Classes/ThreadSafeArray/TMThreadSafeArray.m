...
@implementation TMThreadSafeArray {
...
- (instancetype)initWithContentsOfFile:(NSString *)path modelClass:(Class)modelClass
{
    INIT({
        _path = path.copy;
        ...
        _arr = [NSArray yy_modelArrayWithClass:modelClass json:array].mutableCopy;
        if (_arr == nil) {
...
+ (instancetype)arrayWithContentsOfFile:(NSString *)path modelClass:(Class)modelClass
{
    return [[self alloc] initWithContentsOfFile:path modelClass:modelClass];
}
...
- (id)objectAtIndex:(NSUInteger)index {
    LOCK(id obj = index<_arr.count ? [_arr objectAtIndex:index] : nil); return obj;
}
...
- (NSArray *)subarrayWithRange:(NSRange)range {
    NSArray * arr = nil;
    LOCK({
        if (range.location <= _arr.count) {
            if (range.location+range.length >= _arr.count) {
                range.length = _arr.count - range.location;
            }
            
            arr = [_arr subarrayWithRange:range];
        }
    })
    return arr;
}
...
- (id)objectAtIndexedSubscript:(NSUInteger)idx {
    LOCK(id o = idx<_arr.count ? [_arr objectAtIndex:idx] : nil); return o;
}
...
- (void)exchangeObjectAtIndex:(NSUInteger)idx1 withObjectAtIndex:(NSUInteger)idx2 {
    LOCK({
        if (idx1<_arr.count && idx2<_arr.count) {
...
- (BOOL)isEqualToArray:(NSArray *)otherArray {
    ...
    if ([otherArray isKindOfClass:TMThreadSafeArray.class]) {
        TMThreadSafeArray *other = (id)otherArray;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:TMThreadSafeArray.class]) {
        TMThreadSafeArray *other = object;
        BOOL isEqual;
        dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
        dispatch_semaphore_wait(other->_lock, DISPATCH_TIME_FOREVER);
...
{
    LOCK({
        NSArray *array = _arr.copy;
        NSString *path = _path.copy;
        
        if (!_saveQueue) {
            _saveQueue = [[NSOperationQueue alloc] init];
            _saveQueue.maxConcurrentOperationCount = 1;
            _saveQueue.name = [NSString stringWithFormat:@"com.fl.save.%@.%@", _path.lastPathComponent, @((NSInteger)[[NSDate date] timeIntervalSince1970])];
        }
        
        [_saveQueue cancelAllOperations];
        
        [_saveQueue addOperationWithBlock:^{
            NSArray *writeArray = [array yy_modelToJSONObject];
            if (writeArray) {
                ...
                [writeArray writeToFile:path atomically:YES];
            }
        }];
    });
}
